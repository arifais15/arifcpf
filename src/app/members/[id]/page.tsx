      ...
      <div className="bg-white p-3 border border-black shadow-lg flex flex-col md:flex-row items-center justify-between no-print gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4 bg-slate-50 p-2 border border-black rounded-xl shadow-inner">
          <div className="grid gap-0.5">
            <Label className="text-[10px] font-black uppercase text-blue-700 ml-1 tracking-widest">Archive Period</Label>
            <Select 
              value={selectedFY} 
              onValueChange={(fy) => { 
                setSelectedFY(fy); 
          ...
          <div className="grid gap-0.5"><Label className="text-[10px] font-black uppercase text-indigo-700 ml-1 tracking-widest">Range Start</Label><Input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start:e.target.value})} className="h-8 w-32 border border-black text-[10px] font-black bg-white" /></div>
          <ArrowRightLeft className="size-4 text-black opacity-20 mt-4" />
          <div className="grid gap-0.5"><Label className="text-[10px] font-black uppercase text-indigo-700 ml-1 tracking-widest">Range End</Label><Input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end:e.target.value})} className="h-8 w-32 border border-black text-[10px] font-black bg-white" /></div>
        </div>
      </div>
      ...
      <div className="bg-white p-4 md:p-10 shadow-[10px_10px_0px_0px_rgba(0,0,0,0.05)] border-2 border-black max-w-[1500px] mx-auto w-full print-container overflow-x-auto rounded-none">
        ...
        <div className="grid grid-cols-3 border border-black mb-2 text-[10px] font-black min-w-[1050px] tabular-nums bg-white text-black">
          <div className="border-r border-b border-black py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-blue-700 text-[10px] uppercase tracking-tighter font-black w-[80px]">PERSONNEL:</span>
          ...
          <div className="border-b border-black py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-blue-700 text-[10px] uppercase tracking-tighter font-black w-[80px]">POSITION:</span>
            <span className="text-xs flex-1 truncate text-black font-black">{member?.designation}</span>
          </div>
          <div className="border-r border-b border-black py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-amber-700 text-[10px] uppercase tracking-tighter font-black w-[80px]">VAULT ID:</span>
          ...
          <div className="border-r border-black py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-slate-500 text-[10px] uppercase tracking-tighter font-black w-[80px]">OFFICE:</span>
            <span className="text-xs flex-1 truncate text-black font-black">{member?.zonalOffice || "Head Office"}</span>
          </div>
          <div className="border-r border-black py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-rose-700 text-[10px] uppercase tracking-tighter font-black w-[80px]">STATUS:</span>
          ...
          <div className="py-1 px-4 flex gap-4 items-center h-[21px]">
            <span className="text-indigo-700 text-[10px] uppercase tracking-tighter font-black w-[100px]">DATE REGULAR:</span>
        ...
        <table className="w-full text-[8.5px] border-collapse border border-black font-black tabular-nums min-w-[1050px] text-black">
          <thead className="bg-slate-100 border-b border-black uppercase text-center font-black">
            <tr className="border-b border-black">
              <th rowSpan={2} className="border-r border-black p-1 w-[80px] bg-indigo-50 text-indigo-800">Date</th>
              <th rowSpan={2} className="border-r border-black p-1 text-blue-800">Transaction Detail</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-blue-50/50 text-blue-900">Employee<br/>Contrib</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-rose-50/50 text-rose-900">Loan<br/>Disburse</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-emerald-50/50 text-emerald-900">Loan<br/>Repay</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-slate-100 text-black w-[90px]">Loan<br/>Balance</th>
              <th colSpan={2} className="border-r border-black p-1 bg-amber-50/50 text-amber-900">Yield on</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-slate-200 text-black w-[100px]">Total<br/>Equity</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-indigo-50/50 text-indigo-900">PBS<br/>Contrib</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-indigo-50/50 text-indigo-900">Profit on<br/>PBS Contrib</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-slate-200 text-black w-[100px]">Total<br/>Office</th>
              <th rowSpan={2} className="border-r border-black p-1 bg-black text-white w-[120px]">Cumulative<br/>Fund Total</th>
              <th rowSpan={2} className="p-1 no-print bg-slate-100 w-[80px]">Audits</th>
            </tr>
            <tr className="border-b border-black">
              <th className="border-r border-black p-1 bg-amber-50/20">Member<br/>Fund</th>
              <th className="border-r border-black p-1 bg-amber-50/20">CPF Loan</th>
            </tr>
          </thead>
          <tbody>
            {ledgerLogic.rows.map((r: any, idx: number) => (
              <tr key={idx} className={cn("border-b border-black h-[21px] hover:bg-slate-50 transition-colors bg-transparent", r.isOpening && "bg-slate-50/50 italic")}>
                <td className="border-r border-black p-0 text-center font-mono text-[8.5px] border-black border text-indigo-800">{r.summaryDate}</td>
                <td className="border-r border-black p-0 px-2 uppercase truncate max-w-[200px] leading-none text-[8.5px] border-black border">{r.particulars}</td>
                <td className="border-r border-black p-0 px-2 text-right text-[8.5px] border-black border">{r.c1.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-rose-700 text-[8.5px] border-black border">{r.c2.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-emerald-700 text-[8.5px] border-black border">{r.c3.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right bg-slate-50 font-mono text-[8.5px] border-black border">{r.col4.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-[8.5px] border-black border">{r.c5.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-[8.5px] border-black border">{r.c6.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right bg-slate-100 font-bold text-[8.5px] border-black border">{r.col7.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-[8.5px] border-black border">{r.c8.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right text-[8.5px] border-black border">{r.c9.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right bg-slate-100 font-bold text-[8.5px] border-black border">{r.col10.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="border-r border-black p-0 px-2 text-right bg-slate-50 text-[9px] font-bold border-black border">৳ {r.col11.toLocaleString(undefined, { minimumFractionDigits: 2 })}</td>
                <td className="p-0 text-center no-print bg-slate-50/30 border-r border-black border">
                  {!r.isOpening && (
      ...
      <Dialog open={isEntryOpen} onOpenChange={(o) => { setIsEntryOpen(o); if(!o) setEditingEntry(null); }}>
        <DialogContent className="max-w-[1000px] w-[95vw] bg-white p-0 rounded-none shadow-2xl border-2 border-black overflow-hidden max-h-[95vh] flex flex-col font-ledger">
          ...
          <form onSubmit={handleManualSubmit} className="flex-1 overflow-y-auto p-8 space-y-8 text-black bg-white">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 border border-black rounded-xl">
              <div className="space-y-2">
                 ...
                 <Input name="summaryDate" type="date" max="9999-12-31" defaultValue={editingEntry?.summaryDate} required className="h-11 border-black border-2 font-black text-lg bg-white" />
              </div>
              <div className="space-y-2">
                 ...
                 <Input name="particulars" defaultValue={editingEntry?.particulars} required placeholder="E.G. SALARY JULY-2024" className="h-11 border-black border-2 font-black text-xs uppercase bg-white" />
              </div>
            </div>

            <div className="grid grid-cols-3 gap-6">
               <div className="space-y-4 border border-black p-6 bg-slate-50 rounded-xl">
                  ...
                  <div className="space-y-4">
                     <div className="space-y-1.5">
                        ...
                        <Input type="number" step="0.01" value={manualVals.c1||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c1:Number(e.target.value)})} className="h-10 border-black border font-mono font-black text-right" />
                     </div>
                     <div className="space-y-1.5">
                        <Label className="text-[10px] font-black uppercase text-slate-500">Office Contrib (8)</Label>
                        <Input type="number" step="0.01" value={manualVals.c8||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c8:Number(e.target.value)})} className="h-10 border-black border font-mono font-black text-right" />
                     </div>
                  </div>
               </div>
               ...
               <div className="space-y-4 border border-black p-6 bg-slate-50 rounded-xl">
                  ...
                  <div className="space-y-4">
                     <div className="space-y-1.5">
                        ...
                        <Input type="number" step="0.01" value={manualVals.c2||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c2:Number(e.target.value)})} className="h-10 border-rose-600 border font-mono font-black text-right text-rose-700 bg-white" />
                     </div>
                     <div className="space-y-1.5">
                        <Label className="text-[10px] font-black uppercase text-emerald-600">Repayment (3)</Label>
                        <Input type="number" step="0.01" value={manualVals.c3||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c3:Number(e.target.value)})} className="h-10 border-emerald-600 border font-mono font-black text-right text-emerald-700 bg-white" />
                     </div>
                  </div>
               </div>
               ...
               <div className="space-y-4 border border-black p-6 bg-slate-50 rounded-xl">
                  ...
                  <div className="space-y-3">
                     <div className="space-y-1">
                        ...
                        <Input type="number" step="0.01" value={manualVals.c5||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c5:Number(e.target.value)})} className="h-9 border-orange-600 border font-mono font-black text-right text-orange-700 bg-white" />
                     </div>
                     <div className="space-y-1">
                        <Label className="text-[10px] font-black uppercase text-orange-600">Profit Loan (6)</Label>
                        <Input type="number" step="0.01" value={manualVals.c6||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c6:Number(e.target.value)})} className="h-9 border-orange-600 border font-mono font-black text-right text-orange-700 bg-white" />
                     </div>
                     <div className="space-y-1">
                        <Label className="text-[10px] font-black uppercase text-indigo-600">Profit PBS (9)</Label>
                        <Input type="number" step="0.01" value={manualVals.c9||''} onKeyDown={handleNumericKeyDown} onChange={e=>setManualVals({...manualVals, c9:Number(e.target.value)})} className="h-9 border-indigo-600 border font-mono font-black text-right text-indigo-700 bg-white" />
                     </div>
                  </div>
               </div>
            </div>

            <div className="p-6 bg-white text-black border border-black shadow-inner relative rounded-xl">
            ...
            <Button type="submit" className="w-full h-16 font-black uppercase tracking-[0.4em] bg-black text-white shadow-2xl hover:bg-slate-900 border border-white/10 text-base transition-all group shrink-0">
      ...
      <Dialog open={isSettlementOpen} onOpenChange={(o) => { setIsSettlementOpen(o); if(!o) setEditingEntry(null); }}>
        <DialogContent className="max-w-md bg-white border-2 border-black p-0 overflow-hidden shadow-2xl rounded-none">
          ...
          <form onSubmit={handleFinalSettlement} className="p-6 space-y-6">
            ...
            <div className="bg-slate-900 p-6 border border-black space-y-2 text-white rounded-xl">
