...
import { Printer, ArrowLeft, Loader2, Plus, Edit2, Trash2, ArrowRightLeft, Calculator, ShieldCheck } from "lucide-react";
...
export default function MemberLedgerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const ledgerLogic = useMemo(() => {
    if (!summaries) return { 
        rows: [], 
        grand: { c1:0, c2:0, c3:0, c4:0, c5:0, c6:0, c7:0, c8:0, c9:0, c10:0, c11:0 } 
    };
    ...
    const inRange = allCalculated.filter(r => { 
    ...
    return { 
        rows: displayRows, 
        grand: { ...grandTotals, c4: last.col4, c7: last.col7, c10: last.col10, c11: last.col11 } 
    };
  }, [summaries, dateRange]);
    ...
    <div className="p-8 flex flex-col gap-6 bg-white min-h-screen font-ledger text-[#000000]">
      <PageHeaderActions>
        ...
        <div className="flex items-center gap-3 bg-slate-50 p-2 border-2 border-black rounded-xl no-print shadow-sm">
          <Select value={selectedFY} onValueChange={(fy) => { 
              setSelectedFY(fy); 
            ...
            <SelectTrigger className="h-8 w-[120px] font-black text-[10px] uppercase border-black focus:ring-0"><SelectValue /></SelectTrigger>
          ...
          <Input type="date" value={dateRange.start} onChange={(e) => setDateRange({...dateRange, start: e.target.value})} className="h-8 w-32 border-black text-[10px] font-black focus:ring-0" />
          <ArrowRightLeft className="size-3 opacity-30" />
          <Input type="date" value={dateRange.end} onChange={(e) => setDateRange({...dateRange, end: e.target.value})} className="h-8 w-32 border-black text-[10px] font-black focus:ring-0" />
        </div>
        <div className="flex gap-2 ml-auto no-print">
          <Button variant="outline" onClick={() => setIsEntryOpen(true)} className="h-10 border-black border-2 font-black uppercase text-[10px] shadow-sm hover:bg-slate-50 transition-colors"><Plus className="size-4 mr-2" /> Manual Entry</Button>
      ...
      <div className="bg-white p-12 shadow-2xl border-4 border-black max-w-[1400px] mx-auto w-full font-ledger print-container">
        ...
        <div className="grid grid-cols-3 gap-x-8 gap-y-4 mb-8 text-[12px] font-black border-b-4 border-black pb-6">
          ...
          <div className="flex gap-2 border-b border-black/20 pb-1"><span>ACCOUNT STATUS:</span><span className="uppercase">{member?.status || "Active"}</span></div>
          <div className="flex gap-2 border-b border-black/20 pb-1"><span>ZONAL OFFICE:</span><span className="uppercase">{member?.zonalOffice || "HEAD OFFICE"}</span></div>
        ...
        <table className="w-full text-[9px] border-collapse border-2 border-black font-black tabular-nums">
          ...
          <tfoot className="bg-slate-100 font-black border-t-4 border-black text-[9px] print:table-row-group">
      ...
      <Dialog open={isEntryOpen} onOpenChange={setIsEntryOpen}>
        <DialogContent className="border-4 border-black max-w-4xl bg-white p-0 rounded-none shadow-2xl">
          <DialogHeader className="bg-slate-50 p-8 border-b-4 border-black">
            ...
            <DialogDescription className="text-xs font-black uppercase opacity-60">Manual synchronization of personnel financial records</DialogDescription>
          </DialogHeader>
          <form onSubmit={(e) => { 
              e.preventDefault(); 
              ...
              const d = { 
                  ...
                  employeeContribution: Number(f.get("c1")), 
                  loanWithdrawal: Number(f.get("c2")), 
                  loanRepayment: Number(f.get("c3")), 
                  profitEmployee: Number(f.get("c5")), 
                  profitLoan: Number(f.get("c6")), 
                  pbsContribution: Number(f.get("c8")), 
                  profitPbs: Number(f.get("c9")), 
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-2">
                  ...
                  <Input name="summaryDate" type="date" max="9999-12-31" required className="border-2 border-black font-black rounded-none h-11" />
              </div>
              <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase ml-1">Transaction Particulars</Label>
                  <Input name="particulars" placeholder="e.g. Salary Contribution Sept-2024" required className="border-2 border-black font-black rounded-none h-11" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-12 gap-y-6 bg-slate-50 p-8 border-2 border-black shadow-inner">
                <div className="space-y-4">
                    ...
                    <div className="grid gap-4">
                        <div className="flex items-center justify-between gap-4">
                            <Label className="text-[10px] font-black uppercase shrink-0">Col 1: Employee Contribution</Label>
                            <Input name="c1" type="number" step="0.01" defaultValue={0} className="border-black border-2 font-black text-right w-32 rounded-none h-9" />
                        </div>
                        <div className="flex items-center justify-between gap-4">
                            <Label className="text-[10px] font-black uppercase shrink-0">Col 8: PBS Contribution</Label>
                            <Input name="c8" type="number" step="0.01" defaultValue={0} className="border-black border-2 font-black text-right w-32 rounded-none h-9" />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid gap-4">
                        <div className="flex items-center justify-between gap-4">
                            <Label className="text-[10px] font-black uppercase shrink-0 text-rose-600">Col 2: Loan Withdrawal</Label>
                            <Input name="c2" type="number" step="0.01" defaultValue={0} className="border-rose-600 border-2 font-black text-right w-32 rounded-none h-9" />
                        </div>
                        <div className="flex items-center justify-between gap-4">
                            <Label className="text-[10px] font-black uppercase shrink-0 text-emerald-600">Col 3: Loan Repayment</Label>
                            <Input name="c3" type="number" step="0.01" defaultValue={0} className="border-emerald-600 border-2 font-black text-right w-32 rounded-none h-9" />
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-4 col-span-2">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div className="space-y-1">
                            ...
                            <Input name="c5" type="number" step="0.01" defaultValue={0} className="border-black border-2 font-black text-right rounded-none h-9" />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-[9px] font-black uppercase">Col 9: Profit (PBS)</Label>
                            <Input name="c9" type="number" step="0.01" defaultValue={0} className="border-black border-2 font-black text-right rounded-none h-9" />
                        </div>
                        <div className="space-y-1">
                            <Label className="text-[9px] font-black uppercase">Col 6: Profit (Loan)</Label>
                            <Input name="c6" type="number" step="0.01" defaultValue={0} className="border-black border-2 font-black text-right rounded-none h-9" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-blue-50 p-4 border-2 border-blue-200 flex gap-3 items-start">
                ...
                <p className="text-[10px] font-black uppercase text-blue-800 leading-tight">Voucher values will be instantly reflected in the member's subsidiary ledger and institutional control aggregates. Ensure amounts match the salary payroll sheet.</p>
            </div>
