  ...
  return (
    <div className="p-8 flex flex-col gap-10 bg-background min-h-screen">
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-extrabold text-primary tracking-tight">Executive Summary</h1>
        <p className="text-muted-foreground font-medium uppercase tracking-widest text-xs opacity-70">Contributory Provident Fund Management Terminal</p>
      </div>

      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {stats.map((stat) => (
          <Card key={stat.title} className="border-none shadow-lg bg-card hover:translate-y-[-4px] transition-all duration-300">
            ...
            <CardContent>
              ...
              <p className="text-[10px] font-bold text-muted-foreground uppercase mt-2 opacity-60">{stat.sub}</p>
            </CardContent>
          </Card>
        ))}
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        <Card className="lg:col-span-8 border-none shadow-xl overflow-hidden rounded-2xl">
          <CardHeader className="bg-slate-50 border-b flex flex-row items-center justify-between py-5">
            <div>
              ...
              <CardDescription className="text-xs font-medium">Verified double-entry accounting records</CardDescription>
            </div>
            <Badge variant="outline" className="bg-white border-slate-200">Live Sync</Badge>
          </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-muted/30 border-b">
                  ...
                  <TableHead className="font-bold py-4">Transaction Details</TableHead>
              ...
              <TableBody>
                {recentEntries.length === 0 ? (
                  <TableRow><TableCell colSpan={4} className="text-center py-16 text-slate-400 italic">No historical transactions found.</TableCell></TableRow>
                ) : recentEntries.map((entry) => (
                  <TableRow key={entry.id} className="hover:bg-slate-50/50">
                    <TableCell className="font-mono text-xs font-bold">{entry.entryDate}</TableCell>
                    <TableCell className="text-sm font-semibold text-slate-700">{entry.description}</TableCell>
                    <TableCell><Badge variant="secondary" className="text-[10px] uppercase">{entry.referenceNumber || "AUTO"}</Badge></TableCell>
                    <TableCell className="text-right font-black text-primary">{(entry.totalAmount || 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
        ...
        <Card className="lg:col-span-4 border-none shadow-xl bg-white rounded-2xl flex flex-col">
          <CardHeader className="py-5">
            <CardTitle className="text-lg font-black flex items-center gap-3">
              <PieChart className="size-6 text-accent" />
              Asset Allocation
            </CardTitle>
            <CardDescription className="text-xs font-medium">Diversification of Fund Portfolio</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 flex flex-col gap-8 pt-4">
            ...
            <div className="mt-auto p-5 bg-accent/5 rounded-2xl border border-accent/10">
               <p className="text-[10px] uppercase font-black text-accent tracking-widest mb-2 flex items-center gap-2">
                 <ShieldCheck className="size-3" /> Integrity Check
               </p>
               <p className="text-[11px] text-slate-600 leading-relaxed italic font-medium">Metrics are derived dynamically from verified ledger balances across all investment portfolios and cash accounts.</p>
            </div>
          </CardContent>
        </Card>
      </div>
